# -*- coding: utf-8 -*-
"""
Created on Tue Apr  1 17:47:20 2025

PPO implementation. 

Based on:
    https://github.com/realwenlongwang/PPO-Single-File-Notebook-Implementation
...
class PPO():
    ...
    def train(self, env, max_timesteps, eval_freq, n_eval_episodes, 
              ...
              fisher_freq=None, fisher_steps=1024):
        ...
        # task changes
        self.env.reset_global_step()
        
        if train_seed is not None:
            self.env.np_random = np.random.default_rng(train_seed)
            self.env.action_space.seed(train_seed)
            self.env.observation_space.seed(train_seed)
        ...
        for label in eval_scenarios:
            ...
            params.update(self.env.config["scenarios"][label])
    ...
    def evaluate(self):
        ...
        for label, env in self.eval_envs.items():
            # Seeding for evaluation purpose
            if self.eval_seed is not None:
                env.np_random = np.random.default_rng(self.eval_seed)
                env.action_space.seed(self.eval_seed)
                env.observation_space.seed(self.eval_seed)
